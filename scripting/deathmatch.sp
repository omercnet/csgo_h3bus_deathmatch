...
UpdateState()
{
    ...
    g_bConfig_Filter_FriendlyAimPunch   = cvars_GetPluginConvarBool(eCvars_dm_filter_friendly_aimpunch);
    g_bConfig_Filter_AllAimPunch        = cvars_GetPluginConvarBool(eCvars_dm_filter_all_aimpunch);
...
public Action:OnTraceAttack(victim, &attacker, &inflictor, &Float:damage, &damagetype, &ammotype, hitbox, hitgroup)
{
    ...
    if(!g_bConfig_mp_teammates_are_enemies && g_bConfig_Filter_FriendlyAimPunch && players_IsClientValid(attacker) && GetClientTeam(victim) == GetClientTeam(attacker))
    {
        return Plugin_Handled;
    }
